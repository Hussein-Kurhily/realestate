...
class RentRealestate extends Model
{
    use HasFactory;

    protected $table = 'rent_realestate_table';
    ...
    public function createRecord(array $data, int $id, array $images): RentRealestate {
        return RentRealestate::create([
            'user_id' => $id,
        'phone' => $data['phone'],
        'type' => $data['type'],
        'city' => $data['city'],
...
/**
     ...
     *
     * @param int $id
     * @return RentRealestate|null
     */
    public function findRealEstateById(int $id) : RentRealestate {
        // البحث عن الطلب بواسطة المعرف
        try {
            return RentRealestate::find($id);
        }catch (\Exception $e) {
    ...
    public function deleteRealEstate(int $id) : void {
        try{
            $realEsate = RentRealestate::findRentRealestateById($id);
            $realEsate->delete();
        }catch (\Exception $e) {
            Log::error('Error deleting order: ' . $e->getMessage());
        }
